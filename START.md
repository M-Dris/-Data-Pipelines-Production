...
Tester les évènements `streaming_events_pipeline` : `docker-compose exec -T airflow-webserver python /opt/airflow/tests/integration/verify_streaming_events_condition.py`

Tester combien de documents sont stocké dans listening_events : `docker-compose exec -T postgres psql -U spotify -d spotify -tA -c "SELECT COUNT(*) FROM listening_events;"`